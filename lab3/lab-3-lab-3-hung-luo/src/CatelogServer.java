...
public class CatelogServer {
    ...
    private com.dslab2.IP frontEndIP;
    ...
    public void invalidate(int itemNumber) throws Exception {
        String reqResult ="";
        String displayResult = "";

        // server push
        try {
            HttpDelete invalidate = new HttpDelete("http://" + frontEndIP.toString() + "/invalidate/" + itemNumber);
            CloseableHttpClient httpClient = HttpClients.createDefault();
            CloseableHttpResponse response = httpClient.execute(invalidate);
            reqResult = EntityUtils.toString(response.getEntity());

            if( reqResult.contains("true") ){
                displayResult = "invalidate item " + itemNumber + " success";
            } else{
                displayResult = "invalidate item " + itemNumber + " fail";
            }
    ...
    public CatelogServer withFrontEndIP(String addr){
        frontEndIP = new com.dslab2.IP().withString(addr);
        return this;
    }
